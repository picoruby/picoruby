...
c_mbedtls_cipher_new(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  D("Cipher.new: cipher_name='%s'", cipher_name);
  Mbedtls_cipher_type_key_iv_len(cipher_name, &cipher_type, &key_len, &iv_len);
  D("Cipher.new: cipher_type=%d, key_len=%d, iv_len=%d", cipher_type, key_len, iv_len);
  if (cipher_type == 0) {
    D("Cipher.new: unsupported cipher suite '%s'", cipher_name);
