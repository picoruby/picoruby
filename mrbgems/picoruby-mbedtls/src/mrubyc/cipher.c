...
c_mbedtls_cipher_new(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_new(self.instance->data, cipher_type, MBEDTLS_OPERATION_NONE, key_len, iv_len);
...
{
  MbedTLS_cipher_encrypt(v->instance->data);
}

static void
c_mbedtls_cipher_decrypt(mrbc_vm *vm, mrbc_value *v, int argc)
{
  MbedTLS_cipher_decrypt(v->instance->data);
}

static void
c_mbedtls_cipher_key_len(mrbc_vm *vm, mrbc_value *v, int argc)
{
  uint8_t key_len = MbedTLS_cipher_key_len(v->instance->data);
  SET_INT_RETURN(key_len);
}
...
c_mbedtls_cipher_key_eq(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_key_eq(v->instance->data, (const char *)key.string->data, key.string->size);
...
{
  uint8_t iv_len = MbedTLS_cipher_iv_len(v->instance->data);
  SET_INT_RETURN(iv_len);
}
...
c_mbedtls_cipher_iv_eq(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_iv_eq(v->instance->data, (const char *)iv.string->data, iv.string->size);
...
c_mbedtls_cipher_update_ad(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_update_ad(v->instance->data, (char *)input.string->data, input.string->size);
...
c_mbedtls_cipher_update(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_update(v->instance->data, (char *)input.string->data, input.string->size, output, &out_len);
...
c_mbedtls_cipher_finish(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_finish(v->instance->data, output, &out_len);
  if (ret != 0) {
...
c_mbedtls_cipher_write_tag(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_write_tag(v->instance->data, (const char *)tag, tag_len);
...
c_mbedtls_cipher_check_tag(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  int ret = MbedTLS_cipher_check_tag(v->instance->data, (const char *)input.string->data, input.string->size);
  if (ret == CIPHER_CHECK_TAG_FAILED) {
    mrbc_incref(&v[0]);
