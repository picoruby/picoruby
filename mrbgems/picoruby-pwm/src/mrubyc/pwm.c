...
c_frequency(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  if (GET_TT_ARG(1) == MRBC_TT_FLOAT) {
    ...
    freq = (picorb_float_t)GET_INT_ARG(1);
  } else {
...
static void
set_duty(mrbc_value *v, picorb_float_t duty)
{
  if (duty < 0.0) {
    duty = 0.0;
  } else if (100.0 < duty) {
...
c_duty(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  if (GET_TT_ARG(1) == MRBC_TT_FLOAT) {
    ...
    duty = (picorb_float_t)GET_INT_ARG(1);
  } else {
    mrbc_raise(vm, MRBC_CLASS(TypeError), "wrong argument type");
    return;
  }
...
c_pulse_width_us(mrbc_vm *vm, mrbc_value *v, int argc)
{
  ...
  picorb_float_t duty = (picorb_float_t)GET_INT_ARG(1) / 10000.0 * GETIV(frequency).d;
  set_duty(v, duty);
}
...
{
  mrbc_class *mrbc_class_PWM = mrbc_define_class(vm, "PWM", mrbc_class_object);
  mrbc_define_method(vm, mrbc_class_PWM, "_init", c__init);
  mrbc_define_method(vm, mrbc_class_PWM, "frequency", c_frequency);
  mrbc_define_method(vm, mrbc_class_PWM, "period_us", c_period_us);
  mrbc_define_method(vm, mrbc_class_PWM, "duty", c_duty);
  mrbc_define_method(vm, mrbc_class_PWM, "pulse_width_us", c_pulse_width_us);
}
