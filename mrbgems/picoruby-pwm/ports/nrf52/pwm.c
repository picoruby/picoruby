...
/* EasyDMA reads the sequence out of RAM while the sequence plays, so the
   duty value has to outlive the call that sets it and the CPU is not its
   only accessor -- one live cell per instance. */
