...
mrb_next_executable(mrb_state *mrb, mrb_value self)
{
  ...
  if (executables[i].path) {
    ...
    mrb_value path = mrb_str_new_static(mrb, executables[i].path, strlen(executables[i].path));
    ...
    mrb_value code_val = mrb_str_new_static(mrb, (const char *)vm_code, codesize);
    mrb_hash_set(mrb, hash,
