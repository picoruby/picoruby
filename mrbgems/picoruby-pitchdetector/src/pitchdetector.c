...
// Volume threshold (shared with platform layer)
static uint16_t volume_threshold = 50;

// Enhanced signal power calculation with DC offset removal
float
PITCHDETECTOR_calculate_signal_power(uint16_t *buffer, int size, float *dc_offset)
{
  float sum = 0;
  float mean = 0;
...
void
PITCHDETECTOR_init_processing(void)
{
  // Reset static state
  noise_estimate = 0;
  history_index = 0;
...
float
PITCHDETECTOR_process_buffer(uint16_t *buffer, int size)
{
  ...
  float dc_offset;
  float signal_power = PITCHDETECTOR_calculate_signal_power(buffer, size, &dc_offset);
  ...
  float filtered_power = PITCHDETECTOR_calculate_signal_power(buffer, size, &dc_offset);
