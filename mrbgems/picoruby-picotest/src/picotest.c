 ...
 * ]
 */
static mrbc_value double_methods = mrbc_nil_value();

static bool
search_return_value(struct VM *vm, intptr_t doubled_obj_id, mrbc_sym called_method_id, mrbc_value *return_value)
{
  for (int i = 0 ; i < double_methods.array->n_stored; i++) {
    mrbc_value double_method = double_methods.array->data[i];
...
//  mrbc_incref(&v[0]);
  mrbc_incref(&double_methods);
  SET_RETURN(return_value);
}

static void
c_get_double_methods(struct VM *vm, mrbc_value v[], int argc)
{
  if (double_methods.tt == MRBC_TT_NIL) {
    double_methods = mrbc_array_new(vm, 0);
    mrbc_incref(&double_methods);
  }
  mrbc_incref(&double_methods);
  SET_RETURN(double_methods);
}
...
static void
c_double__define_method(struct VM *vm, mrbc_value v[], int argc)
{
...
mrbc_picotest_init(mrbc_vm *vm)
{
  ...
  mrbc_define_method(vm, class_Picotest_Double, "get_double_methods", c_get_double_methods);
  mrbc_define_method(vm, class_Picotest_Double, "_alloc", c_double__alloc);
  mrbc_define_method(vm, class_Picotest_Double, "_define_method", c_double__define_method);
