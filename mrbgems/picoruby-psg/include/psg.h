...
typedef enum {
  ...
  PSG_PKT_SOUND_STEP    = 6   // generation, voice + volume + flags, noise_period, tone_period
} psg_opcode_t;
...
typedef struct {
  ...
  // Sound step generation. Used to ignore stale queued sound steps.
  uint8_t sound_generation;
} psg_t;
