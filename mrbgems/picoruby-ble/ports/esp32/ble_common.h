...
// VM-thread only: copies dynamic readable attributes into plain memory so gatt_access_cb can serve GATT reads on the host task without touching the GC-managed heap.
void picoruby_ble_refresh_read_mirrors(void);
