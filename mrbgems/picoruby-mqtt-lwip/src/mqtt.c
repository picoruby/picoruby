...
#define Net_busy_wait_ms(ms) usleep(ms * 1000)
#define Net_get_ip(host, ip) (-1)
#endif
...
static bool poll_state() {
  ...
  default:
    if (g_ctx.fsm_state == MQTT_STATE_CONNECTING) {
      ...
      if (poll_count % 100 == 0) {
        console_printf(
            "[MQTT POLL] Still in CONNECTING state, poll count: %d\n",
            poll_count);
      }
    }
    break;
...
int MQTT_connect_impl(const char *host, int port, const char *client_id) {
  console_printf(
      "[MQTT] MQTT_connect_impl called: host=%s, port=%d, client_id=%s\n", host,
      port, client_id);
  ...
  console_printf("[MQTT] About to enter while loop: state=%d (ACTIVE=%d, "
                 "ERROR=%d), timeout=%d\n",
                 g_ctx.fsm_state, MQTT_STATE_ACTIVE, MQTT_STATE_ERROR, 10000);
  int timeout_ms = 10000;
  int elapsed = 0;
  while (g_ctx.fsm_state != MQTT_STATE_ACTIVE &&
         g_ctx.fsm_state != MQTT_STATE_ERROR && timeout_ms > 0) {
#ifdef PICO_BUILD
    cyw43_arch_poll();
#endif
    poll_state();
  ...
  console_printf("[MQTT] Connection wait finished. Final state: %d (ACTIVE=%d, "
                 "ERROR=%d)\n",
...
static void mqtt_request_cb(void *arg, err_t err) {
  mqtt_context_t *ctx = (mqtt_context_t *)arg;
  if (err == ERR_OK) {
    if (ctx->fsm_state == MQTT_STATE_SUBSCRIBING ||
        ctx->fsm_state == MQTT_STATE_PUBLISHING) {
      ctx->fsm_state = MQTT_STATE_ACTIVE;
    }
  } else {
    ctx->fsm_state = MQTT_STATE_ERROR;
