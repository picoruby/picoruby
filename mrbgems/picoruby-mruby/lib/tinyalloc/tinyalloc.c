...
/*
 *  functions by hasumikin
 */

void
ta_walk_pool(struct walker_data *data)
{
  size_t header_size = sizeof(Block) - sizeof(void *);
  Block *block = heap->used;
  data->total = (size_t)heap_limit - (size_t)heap + 1;
  while (block) {
    data->used += block->size + header_size;
    block = block->next;
  }
  data->used += sizeof(Heap);
  block = heap->free;
...
void *
ta_realloc(void *p, size_t size)
{
  size_t old_size = ta_usable_size(p);
  if (size <= old_size) {
    return p;
  }
  void *new_ptr = ta_alloc(size);
  if (new_ptr == NULL) {
    return NULL;
  }
