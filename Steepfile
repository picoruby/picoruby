...
#  )
  ...
  Dir.glob("**/sig/").each do |dir|
    ...
    unless dir.include?("lib/prism") || dir.include?("build/repos") || dir.include?(MRUBYC_SIG) || dir.include?("-prk-") || dir.include?("vendor/")
      signature dir
    end
  end
  ...
  ignore "mrbgems/picoruby-shell/shell_executables/sl.rb"
